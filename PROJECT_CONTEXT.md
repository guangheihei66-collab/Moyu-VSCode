...
Moyu VS Code is a local-only VS Code leisure center for Windows 10/11. The
current 0.2.1 product contains a local novel reader, Settings, and a
reversible in-Webview Boss Mode; the former 2048 feature has been removed.

## Current phase

V1 implementation and the UI redesign release checkpoint are complete on
`feature/moyu-v1-implementation`. The local 0.2.1 bounded-removal checkpoint
is now implemented: the Sidebar has three entries (Home, Books, Settings),
the main panel retains Home/Books/Reader/Settings and neutral Boss Mode, and
the 2048 production feature is absent. Reader/EPUB fixture-driven visual
checks remain explicitly open because no manual book fixture is shipped; their
automated Host/Webview coverage is green. Follow-up notes remain in
...
- One main WebviewPanel per VS Code window hosts the reader, Settings, and Boss
  overlay.
...
- Task 22 now provides isolated current/minimum VS Code Extension Host lanes, deterministic TEMP fixtures, TXT/EPUB/Boss/Webview/lifecycle acceptance coverage, competing child-process transaction coverage, and a manual Windows checklist.
...
- UI Redesign Tasks 1–11 provided the approved presentation layer across the
  shared components, native Sidebar, Home, Bookshelf, Reader, EPUB drawer,
  Settings, neutral Boss overlay, responsive layouts, accessibility, themes,
  and integrated route/lifecycle regression coverage. The 0.2.1 bounded change
  removed the former 2048 surface and its production domain, protocol,
  persistence, and controller paths while preserving the shared boundaries.
- UI Redesign Task 12 packaged version `0.2.0`; the current 0.2.1 checkpoint
  retains its runtime-only VSIX boundary and removes all 2048 production assets
  and commands.

## Pending work

- Complete a fixture-backed manual Reader TXT, EPUB drawer, and Reader Boss
  visual pass when an approved non-private fixture is available.
...
- The `0.2.1` bounded removal is released locally only; no push, Marketplace
